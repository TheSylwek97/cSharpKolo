/*Wielomian, to znane Ci pojęcie z matematyki ( Wielomian @Wikipedia ):
...
Wielomian jest jednoznacznie reprezentowany przez ciąg jego współczynników: a_n, a_{n-1}, ..., a_2, a_1, a_0a 
n
​	
 ,a 
n−1
...
Stopień wielomianu to wartość najwyższej potęgi stojącej przy xx. Wielomian stopnia zerowego to tylko liczba, będąca wyrazem wolnym a_0a 
0
​	
 .
...
Klasa opisuje wielomian stopnia dowolnego (to znaczy zarówno wielomian stopnia 0, stopnia 2 jak i np. stopnia 100).

Obiekty klasy Wielomian są niezmiennicze.

Wielomian domyślny to wielomian zerowy - tzn. stopnia zerowego, tylko wyraz wolny a_0 = 0a 
0
​	
 =0.
...
Polecenia
Utwórz solution składające się z 3 projektów: (1 pkt.)
typu class library, w którym zlokalizowana będzie klasa Wielomian.
typu unit test (MSTestV2) w którym umieścisz testy jednostkowe wybranych publicznych funkcjonalności klasy Wielomian.
typu console application zawierający poniższy kod sprawdzający poprawność implementacji klasy.
W projekcie typu console application, w pliku Program.cs, wklej podany poniżej kod. Kod ten testuje poprawność wykonania poszczególnych poleceń.
...
Podanego kodu w projekcie console app NIE WOLNO CI ZMIENIAĆ. Oczywiście, w trakcie implementacji, możesz komentować i odkomentować wybrane fragmenty kodu, sprawdzając poprawność Twoich rozwiązań.
...
Tworzenie wielomianu: (1 pkt.)

Domyślnym wielomianem jest wielomian zerowy.
Tworzenie wielomianu odbywa się na podstawie podania listy wartości współczynników - od tego, stojącego przy najwyższej potędze ( a_na 
n
​	
  ) do wyrazu wolnego ( a_0a 
0
​	
  ).
Przykładowo, wywołanie konstruktora Wielmian(2, -3, 0, 1, -2) spowoduje utworzenie obiektu odpowiadającemu "matematycznemu wielomianowi": 2x^4 -3x^3 +x -22x 
4
 −3x 
3
 +x−2.
...
Przykładowo, dla tablicy int[] a {2, -3, 0, 1, -2} utworzony zostanie obiekt odpowiadający "matematycznemu wielomianowi": 2x^4 -3x^3 +x -22x 
4
 −3x 
3
 +x−2.
...
Przykładowo, dla tablicy int[] a {0, 0, 0, 2, -3, 0, 1, -2} utworzony zostanie obiekt odpowiadający "matematycznemu wielomianowi": 2x^4 -3x^3 +x -22x 
4
 −3x 
3
 +x−2.
...
Reprezentacja ToString() (1 pkt.)

Zapewnij reprezentację tekstową wielomianu zgodną z notacją algebraiczną (akceptowalną przez większość CAS - Computer Algebra Systems) podanymi przykładami:
wielomian stopnia zero: np. dla W(2) zwróć "2"
wielomian stopnia > 2: np. dla W(3,-2,1) zwróć "3x^2 - 2x^1 + 1"
Równość wielomianów (1 pkt.)
...
Operacje arytmetyczne na wielomianach (1 pkt.)
...
Operacje konwersji (1 pkt.)

Zdefiniuj konwersję niejawną (implicit) z typu int na Wielomian
Zdefiniuj konwersję jawną (explicit) z typu Wielomian:
...
Przeglądanie wielomianu - indexer (1 pkt.)

Zaimplementuj mechanizm przeglądania (tylko do odczytu, bo wielomian ma być immutable) współczynników wielomianu poprzez odwołanie się do indeksów (w[i] oznacza i-ty współczynnik wielomianu w).
Przeglądanie wielomianu - pętla foreach (1 pkt.)
...
Metoda parsująca ze string (1 pkt.)
...
Pamiętaj o zapewnieniu pełnej niezmienniczości obiektom klasy Wielomian oraz o zapieczętowaniu klasy (1 pkt.)

Poniższe polecenia dotyczą projektu testów jednostkowych (1 pkt.)

Utwórz testy jednostkowe dla:
properties Stopien,
operatora dodawania wielomianów.
Poniższe polecenia dotyczą projektu typu console app.

Metoda rozszerzająca - Eval (2 pkt.)
...
Sortowanie listy wielomianów (2 pkt.)
...
Do oceny przesyłasz solution z projektami, skompresowane w formie archiwum ZIP. Przed spakowaniem sprawdź, czy kod się kompiluje. Kod z błędami nie będzie oceniany.
...
współczynnik 0 powoduje, że jednomian w zapisie wielomianu znika (np. 0x^20x 
2
 )
współczynnik 1 jednomianu nie jest zapisywany (np. zamiast 1x^21x 
2
  zapiszemy x^2x 
2
 )
potęga 1 zwyczajowo nie jest zapisywana (np. zamiast x^1x 
1
  zapiszemy xx)
Przykład: Wielomian W(-2,0,1,0) zamiast -2x^3 + 0x^2 + 1x^1 + 0−2x 
3
 +0x 
2
 +1x 
1
 +0 zapiszemy -2x^3 + x−2x 
3
 +x.
...
Zadanie 3-1 LINQ (8 pkt.)
Polecenia 1 oraz 2 związane są z poniższym wprowadzeniem. Polecenie 3 jest niezależne od pozostałych.

W Twojej aplikacji zdefiniowane są klasy Klient oraz Zamowienie.
...
w notacji kropkowej (fluent) (1 pkt.)

w notacji query (1 pkt.)
...
a) Napisz kwerendę LINQ zwracającą liczbę okrążeń zaliczonych przez biegacza. (1 pkt.)
b) Napisz kwerendę LINQ zwracającą sekwencję IEnumerable<TimeSpan> reprezentującą czasy kolejnych okrążeń. (1 pkt.)*/